...
parser = argparse.ArgumentParser(description="Mininet pyswitch lab")
...
class PySwitchTopo(Topo):
    ...
    def __init__(self, args):
        ...
        for node in ['server1','server2','client']:
            self.addLink(node, 'switch', **linkconfig)
...
def setup_addressing(net):
    ...
    switch.cmdPrint('ebtables -F')
    switch.cmdPrint('ebtables -P INPUT DROP')
    switch.cmdPrint('ebtables --list')
    switch.cmdPrint('iptables -F')
    switch.cmdPrint('iptables -P INPUT DROP')
    switch.cmdPrint('iptables --list')
